...
import {
  RemoveSubscriptionFiltersRequest,
...
function run(): void {
  ...
  try {
    ...
    printMutationUsage("remove-subscriptions");
    process.exitCode = 1;
    return;
  }
  ...
  const request = new RemoveSubscriptionFiltersRequest();
  request.setClientId(args.clientId);
  request.setCategoriesList(categories);
  request.setSkillLevelList(selectedSkillLevels);

  client.removeSubscriptionFilters(request, (error, response) => {
    if (error) {
      console.error("Remove-subscriptions RPC failed:", error.message);
      client.close();
      process.exit(1);
      return;
    }

    console.log("Remove-subscriptions result:", {
